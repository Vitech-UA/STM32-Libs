...
int main(void)
{
	...
	/* USER CODE BEGIN 2 */
	I2C_ScanBus();
	sprintf((char*) LCD_BUFFER, "STM32 AD9833\r\n");
	HAL_UART_Transmit(&huart2, (uint8_t*) LCD_BUFFER, strlen(LCD_BUFFER),
	HAL_MAX_DELAY);


	ad9833_set_power(POWER_ON);
	ad9833_set_wave_param(TRIANGLE,100, 1);
	...
	while (1)
	{
		ad9833_set_wave_param(SINUS,1000, 1);
		HAL_Delay(2000);
		ad9833_set_wave_param(SQUARE,1500, 1);
		HAL_Delay(2000);
		ad9833_set_wave_param(TRIANGLE,1250, 1);
		HAL_Delay(2000);
		/* USER CODE END WHILE */
...
static void MX_GPIO_Init(void)
{
	...
	/*Configure GPIO pin : SQW_PIN_Pin */
	GPIO_InitStruct.Pin = SQW_PIN_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_IT_FALLING;
	GPIO_InitStruct.Pull = GPIO_PULLUP;
	HAL_GPIO_Init(SQW_PIN_GPIO_Port, &GPIO_InitStruct);
