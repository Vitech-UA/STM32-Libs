...
bool	W25qxx_Init(void)
{
	...
	sprintf(UART_BUFFER, "Init Begin\r\n");
	HAL_UART_Transmit(&huart1, UART_BUFFER, strlen(UART_BUFFER), 100);
	#endif
	id=W25qxx_ReadID();
	
	#if (_W25QXX_DEBUG==1)
	sprintf(UART_BUFFER,"w25qxx ID:0x%X\r\n",id);
	HAL_UART_Transmit(&huart1, UART_BUFFER, strlen(UART_BUFFER), 100);
	...
	*/
	...
	sprintf(UART_BUFFER,"Page Size: %d Bytes\r\nPage Count: %d\r\nSector Size: %d Bytes\r\nSector Count: %d\r\nBlock Size: %d Bytes\r\nBlock Count: %d\r\nCapacity: %d KiloBytes\r\n",
			 w25qxx.PageSize,w25qxx.PageCount,w25qxx.SectorSize,w25qxx.SectorCount,w25qxx.BlockSize,w25qxx.BlockCount,w25qxx.CapacityInKiloByte);
	HAL_UART_Transmit(&huart1, UART_BUFFER, strlen(UART_BUFFER), 100);
...
void 	W25qxx_WriteSector(uint8_t *pBuffer	,uint32_t Sector_Address,uint32_t OffsetInByte	,uint32_t NumByteToWrite_up_to_SectorSize)
{
    ...
    volatile char UART_BUFFER[512]={};
	sprintf(UART_BUFFER, "+++w25qxx WriteSector:%d, Offset:%d ,Write %d Bytes, begin...\r\n",Sector_Address,OffsetInByte,NumByteToWrite_up_to_SectorSize);
	HAL_UART_Transmit(&huart1, UART_BUFFER, strlen(UART_BUFFER), 100);
	W25qxx_Delay(100);
	#endif	
...
{
	/*Якщо розмір записаних даних більше розміру блоку чи = 0, то розмір записаних даних буде = макс. розміру блоку (64кБ)*/
...
#if (_W25QXX_DEBUG==1)
		sprintf(UART_BUFFER, "---w25qxx WriteBlock Faild!\r\n");
		HAL_UART_Transmit(&huart1, UART_BUFFER, strlen(UART_BUFFER), 100);
		W25qxx_Delay(100);
#endif
		return;
...
#if (_W25QXX_DEBUG==1)
	sprintf(UART_BUFFER,"---w25qxx WriteBlock Done\r\n");
	HAL_UART_Transmit(&huart1, UART_BUFFER, strlen(UART_BUFFER), 100);
	W25qxx_Delay(100);
#endif
}
