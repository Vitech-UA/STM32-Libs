...
#define DC_DATA() HAL_GPIO_WritePin(DC_GPIO_Port, DC_Pin, GPIO_PIN_SET) /* Пін DC вгору для відправки даних */
#define DC_COMMAND() HAL_GPIO_WritePin(DC_GPIO_Port, DC_Pin, GPIO_PIN_RESET) /* Пін DC вниз для відправки команд */
...
void LCD_WriteMultiple(uint8_t *pData, uint32_t size);
uint16_t LCD_Color565(uint8_t r, uint8_t g, uint8_t b);
/* Прототипи функцій рпоботи з текстом */
...
uint16_t ST7789_RandColor(void);


#ifdef USE_DMA
void vFinaLCD_ST7789_ClearScreenDMA(uint8_t x, uint8_t y, uint8_t width, uint8_t height);
void vFinaLCD_ST7789_FillScreenDMA(uint16_t color);
void vFinaLCD_DrawImage240_240_FromFlashDMA(const uint8_t *bmp);
void vFinaLCD_DrawImageFromFlashDMA(uint8_t x, uint8_t y, uint8_t width, uint8_t height, const uint8_t *bmp);
#endif
