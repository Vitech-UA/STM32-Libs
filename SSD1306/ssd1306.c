...
{
	uint32_t i, b, j;
	SSD1306_COLOR tmp = ssd1306_GetColor();
	b = 0;
	if(frame >= img[2])
	{
		return;
	...
	for (i = 0; i < img[1]; i++)
	{
		for (j = 0; j < img[0]; j++)
		{
			...
			ssd1306_SetColor((uint8_t) (img[b/8 + 5 + start] >> (b%8)) & 1);
			ssd1306_DrawPixel(x + j, y + i);
			b++;
		}
	}
	ssd1306_SetColor(tmp);
}
