...
void ST7789_SendData(uint8_t Data) {
	...
	/*Відправка даних через HAL*/
	/* if(HAL_SPI_Transmit(&hspi2, &Data, 1, 100) != HAL_OK) {

	 Error_Handler();
	 }
	 */
	/*Відправка даних через CMSIS*/
...
static void ST7789_DrawLine_Slow(int16_t x1, int16_t y1, int16_t x2, int16_t y2,
		uint16_t color) {
