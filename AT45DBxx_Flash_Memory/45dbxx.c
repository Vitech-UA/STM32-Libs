...
u_int8_t UART_BUFFER[MAX_BUFFER_SIZE];
...
bool AT45dbxx_Init(void)
{
	...
	if (Temp0 == 0x1f)
	{
		...
		case 0x06:	//	AT45db161
			...
			sprintf(UART_BUFFER, "\r\nDetected: AT45DB161\r\n");
			HAL_UART_Transmit(&huart1, UART_BUFFER, strlen(UART_BUFFER), 100);
		...
		/*У кінці роботи функції виводжу всі параметри які вдалось дістати з мікросхеми*/
		sprintf(UART_BUFFER,
				"Flash size: %d Mbit\r\nCount of Pages: %d\r\nPage size: %d Byte\r\n",
				AT45dbxx.FlashSize_MBit, AT45dbxx.Pages, AT45dbxx.PageSize);
		HAL_UART_Transmit(&huart1, UART_BUFFER, strlen(UART_BUFFER), 100);
		return true;
	}
	else
...
void AT45dbxx_EraseChip(void)
{
...
#if (_45BDXX_USE_UART_DEBUG == 1)
	HAL_UART_Transmit(&huart1, "Erasing started...Please wait\r\n",
			strlen("Erasing started...Please wait\r\n"), 100);
#endif
	...
	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);
	AT45dbxx_WaitBusy();
	/*Відладочний вивод у термінал*/
#if (_45BDXX_USE_UART_DEBUG == 1)
	HAL_UART_Transmit(&huart1, "Erasing Done\r\n", strlen("Erasing Done\r\n"),
			100);
#endif
}
//################################################################################################################
...
{
	HAL_UART_Transmit(&huart1, "Start writing Block...\r\n",
			strlen("Start writing Block...\r\n"), 100);

    size_t pgs = nblocks;
	while(pgs-- > 0)
	{
      AT45dbxx_WritePage(buf, sizeof(buf), sblock);
      sblock++;
	}
	HAL_UART_Transmit(&huart1, "Stop writing Block...\r\n",
				strlen("Stop writing Block...\r\n"), 100);
}
...
void AT45dbxx_Read_Block(uint32_t sblock, size_t nblocks, uint8_t *buf)
{
	...
	//nb = at45db_read(sblock << __flash.pg_shifts,nblocks << __flash.pg_shifts, buf);
	AT45dbxx_ReadPage(sblock << AT45dbxx.Shift, nblocks << AT45dbxx.Shift, buf);
